...
#home
@DatasetReader.register('nytimes_faces_ner_matched_position_ner')
class NYTimesFacesNERMatchedPositionNERReader(DatasetReader):
    ...
    def _read(self, split: str):
        ...
        for article_id in ids:
            ...
            for pos in image_positions:
                ...
                caption_ner = sections[pos].get('named_entities', [])
                #check the ner distribution
                section_scan = {}

                if caption_ner:
                    caption_words = [x['text'] for x in caption_ner]
                    for k, section in enumerate(sections):
                        if section['type'] == 'paragraph':
                            paragraph_text = section['text']
                            for word in caption_words:
                                if word in paragraph_text:
                                    section_scan[word] = section_scan.get(word, []) + [k]

                ##########################################################
                #Extract the image centered position
                if not section_scan or not caption_ner:
                    before = []
                    after = []
                    i = pos - 1
                    j = pos + 1
                    ...
                    result = []
                    for x in section_scan.values():
                        result += x

                    #First include these section
                    for k in range(min(result) ,max(result)+1):
                        text = sections[k]['text']
                        paragraphs.append(text)
                        named_entities |= self._get_named_entities(sections[k])
                        n_words += len(self.to_token_ids(text))
                        if n_words >= 510:
                            break
                    #Next we get the rest of the words
                    before = []
                    after = []
                    if n_words < 510:
                        i = min(result) - 1
                        j = max(result) + 1
                        while True:
                            if i >= 0 and sections[i]['type'] == "paragraph":
                                text = sections[i]['text']
                                before.insert(0, text)
                                named_entities |= self._get_named_entities(sections[i])
                                n_words += len(self.to_token_ids(text))
                            ...
                            if j < len(sections) and sections[j]['type'] == 'paragraph':
                                text = sections[j]['text']
                                named_entities |= self._get_named_entities(sections[j])
                                after.append(text)
                                n_words += len(self.to_token_ids(text))
                            ...
                            if n_words >= 510 or (i <= 0 and j >= len(sections)):
                                break
